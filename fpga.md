...
- FPGA に送り込むことが難しい場合は、問題をFPGA内のROMに予め書き込んでおいても構いません。
- 上記以外の方法による入力も可能です。

## ハードウェアの出力
- ホストPCとFPGA間で通信を行い、最終盤面のテキストファイルをホストPCに送り出す等の方法により、ホストPC上で解答の確認を行います。
- ホストPC上での確認が難しい場合は、FPGA内のRAMに格納したままで、シミュレーションで確認を行っても構いません。
- 上記以外の方法による出力も可能です。
